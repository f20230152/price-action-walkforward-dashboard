...
def run_objective(bars: pd.DataFrame, params: list[SpikeParams], objective: str) -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    for start_i in range(TRAIN_MONTHS, len(months), TEST_MONTHS):
        ...
        top = train_grid.drop(columns=["params"]).head(50).copy()
        ...
        trades, daily = backtest_spike_strategy(test_bars, selected_params, COST_CENTS, MAX_TRADES_PER_DAY)
    ...
    if test_daily:
        oos_daily = pd.concat(test_daily).sort_index().groupby(level=0).sum()
    else:
    ...
    return decisions_df, oos_daily, trades_df, rankings_df
...
def main() -> None:
    ...
    for objective in ["total_pnl_cents", "daily_sharpe", "profit_factor"]:
        print("running objective", objective, "candidates", len(params), flush=True)
        decisions, daily, trades, ranking = run_objective(bars, params, objective)
    ...
    summary = {
        ...
        "cost_cents_round_trip": COST_CENTS,
        "max_trades_per_day": MAX_TRADES_PER_DAY,
